...
const DropzoneComponent: React.FC<IProps> = (props) => {
  ...
  if (props.values?.[props.name]) {
    const filesOrUrls = props.values[props.name];
    const urls = filesOrUrls.map((item: File | string) => {
      if (typeof item === 'string') {
        // Return existing URL directly
        return item;
      } else {
...
// Also update the handleRemove function to handle both types
const handleRemove = (index: number) => {
  if (props.setFieldValue && props.values) {
    const currentValues: Array<File | string> = [...props.values[props.name]];
